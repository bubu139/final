...
// URL của backend, bạn có thể đặt trong file .env.local sau này
// Ví dụ: NEXT_PUBLIC_API_URL=http://localhost:8000
...
export function TestRenderer({ testData, onRetry, testId, topic, difficulty }: Props) {
  ...
  const checkAnswer = (question: Question, userAnswer: any): boolean => {
    ...
    if (question.type === 'multiple-choice') return safeUserAnswer === (question as any).answer;
    if (question.type === 'true-false') return JSON.stringify(safeUserAnswer) === JSON.stringify((question as any).answer);
    if (question.type === 'short-answer') return safeUserAnswer?.join('') === (question as any).answer;
    return false;
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      // BƯỚC 1: Tính điểm và thu thập dữ liệu (như cũ)
      let correctCount = 0;
      const answeredQuestions = allQuestions.map((q) => {
        ...
        return {
          ...
          isCorrect: isCorrect,
          topic: (q as any).topic || topic, // Đảm bảo có topic
        };
      });
  ...
  const handleTakeAdaptiveTest = () => {
    ...
    const weakTopicsParam = JSON.stringify(topicsToPractice || []);
    router.push(`/tests/adaptive?weakTopics=${encodeURIComponent(weakTopicsParam)}`);
  };
