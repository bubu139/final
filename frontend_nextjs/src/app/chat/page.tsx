...
// page.tsx (Thay thế code ở dòng 134)
// page.tsx (Thay thế khối useEffect xử lý MathJax)
useEffect(() => {
  if (!window.MathJax) return;

  // Sử dụng setTimeout để đảm bảo DOM đã được React render xong hoàn toàn
  const timer = setTimeout(() => {
    // 1. Tìm các nút mindmap
    const mindmapElements = document.querySelectorAll<HTMLElement>('.mathjax-node');

    // 2. Tìm các tin nhắn của trợ lý AI
    const messageElements = document.querySelectorAll<HTMLElement>('[id^="assistant-message-"]');

    // 3. Gộp danh sách
    const allElements: HTMLElement[] = [
      ...Array.from(mindmapElements),
      ...Array.from(messageElements)
    ];

    // 4. Render
    if (allElements.length > 0) {
      // Xóa nội dung cũ của MathJax (nếu có) để tránh render chồng đè
      // (Bước này tùy chọn, nhưng giúp ổn định hơn nếu component re-render nhiều)
      window.MathJax.typesetPromise(allElements).catch((err: any) => console.error('MathJax error:', err));
    }
  }, 10); // Delay 10ms là đủ

  return () => clearTimeout(timer);
}, [input, mindmapUpdates, messages]); // Thêm messages vào dependency để chắc chắn
