...
YÊU CẦU QUAN TRỌNG:
- KHÔNG dùng LaTeX (KHÔNG dùng \\frac, \\sqrt, \\lim, \\int,...).
- KHÔNG dùng ký hiệu $...$, $$...$$.
- KHÔNG dùng ký hiệu escape như \\alpha, \\beta, \\mu, \\infty.
- TẤT CẢ biểu thức toán học phải dùng ký hiệu Unicode thực, ví dụ:
  ✓ x², x³, x⁻¹
  ✓ √x
  ✓ f′(x), f″(x)
  ✓ ∀x ∈ ℝ
...
"""
        ...
        # ========================
        # VALIDATE JSON (ĐÃ SỬA)
        # ========================
        try:
            # Xử lý trường hợp AI trả về ```json ... ```
        ...
        # Sửa: Kiểm tra "parts" thay vì "questions"
...
- Tập trung vào việc giúp học sinh TỰ TIN hơn"""
        ...
        if not json_match:
            ...
            print("Raw response:", result_text[:400])
        ...
        # Loại bỏ các ký tự gây JSONDecodeError
        json_text = re.sub(r"[\x00-\x1F\x7F]", " ", json_text)           # control chars
        json_text = re.sub(r"[\U00010000-\U0010ffff]", "", json_text)    # emoji & unicode extended

        # Chuẩn hoá dấu ngoặc kép
        json_text = json_text.replace("“", "\"").replace("”", "\"")
        json_text = json_text.replace("‘", "'").replace("’", "'")

        # Loại xuống dòng thật và tab
        json_text = json_text.replace("\n", " ").replace("\t", " ")

        # Loại escape thừa dạng "\\n" và "\\t"
        json_text = json_text.replace("\\n", " ").replace("\\t", " ")

        # Xử lý trường hợp AI bọc JSON trong ```json ... ```
        if json_text.startswith("```json"):
            json_text = json_text[7:]
        elif json_text.startswith("```"):
            json_text = json_text[3:]
